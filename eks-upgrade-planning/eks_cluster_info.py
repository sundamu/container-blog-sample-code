...
def get_cluster_info(cluster_name, region, profile=None):
    ...
    try:
        ...
        print(f"Unexpected error getting cluster info: {e}")
        raise
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logging.exception("Error deleting temporary certificate file", e, stack_info=True, exc_info=True)  # import logging
    except Exception as e:
...
def collect_cluster_info(cluster_name, region, target_version, profile=None, connect_k8s=False):
    """
    收集集群信息并返回一个包含所有信息的字典
    
    Args:
        cluster_name (str): EKS集群名称
        region (str): AWS区域
        target_version (str): 目标EKS版本
        profile (str, optional): AWS配置文件
        connect_k8s (bool, optional): 是否连接到Kubernetes API服务器
    ...
    if region not in ["cn-north-1", "cn-northwest-1"]:
        ...
        result["deprecated_apis"] = deprecated_apis
        
        # Get addon compatibility issues
        addon_comp_issues = get_addon_compatibility_issues(compatibility_issues)
        result["addon_compatibility_issues"] = addon_comp_issues
    else:
        result["compatibility_issues"] = []
        result["deprecated_apis"] = []
        result["addon_compatibility_issues"] = []
    
    nodegroups, min_nodegroup_version = get_nodegroups(cluster_name, region, profile)
    result["nodegroups"] = nodegroups
    result["min_nodegroup_version"] = min_nodegroup_version
    
    fargate_profiles = get_fargate_profiles(cluster_name, region, profile)
    result["fargate_profiles"] = fargate_profiles
    
    installed_addons = get_installed_addons(cluster_name, region, profile)
    result["installed_addons"] = installed_addons
    
    addon_upgrade_info, upgrade_recommended = get_addon_upgrade_info(cluster_name, region, current_version, target_version, profile)
    result["addon_upgrade_info"] = addon_upgrade_info
    result["upgrade_recommended"] = upgrade_recommended
    ...
    result["kube_proxy_version"] = kube_proxy_version
...
def collect_cluster_info(cluster_name, region, target_version, profile=None, connect_k8s=False):
    ...
    if connect_k8s:
        ...
        if api_client:
            min_self_managed_version, min_karpenter_version, self_managed_count, karpenter_count = get_node_versions(api_client)
            result["min_self_managed_version"] = min_self_managed_version
            result["min_karpenter_version"] = min_karpenter_version
            result["self_managed_count"] = self_managed_count
            result["karpenter_count"] = karpenter_count
        
            # Get open source addon information
            opensource_addons = get_opensource_addons(api_client, installed_addons)
            result["opensource_addons"] = opensource_addons
            
            # Get core component version information (if not in EKS addon list)
            core_components = get_core_components_version(api_client, installed_addons)
            result["core_components"] = core_components
        else:
            print("Failed to connect to kube-apiserver!")
            sys.exit(1)
    
    # Calculate version skew recommendations
    if kube_proxy_version and min_nodegroup_version:
        version_skew_recommended, recommendations = check_version_skew(current_version, min_nodegroup_version, kube_proxy_version, target_version)
        result["version_skew_recommended"] = version_skew_recommended
        result["version_skew_recommendations"] = recommendations
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-file", help="Output file path to save cluster information as JSON")
    ...
    if args.output_file:
        try:
            with open(args.output_file, 'w', encoding='utf-8') as f:
                json.dump(cluster_info, f, indent=2, cls=DateTimeEncoder)
            print(f"Cluster information saved to {args.output_file}")
        except Exception as e:
            print(f"Error saving cluster information to file: {e}")
            sys.exit(1)
    else:
        # 写到标准输出
        print(json.dumps(cluster_info, indent=2, cls=DateTimeEncoder))
